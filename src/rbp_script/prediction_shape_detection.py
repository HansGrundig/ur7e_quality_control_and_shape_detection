...
#classification is handled inline via minAreaRect
...
def get_contours(img, img_dilate, img_contour, area_min, yolo_model): 
    ...
    if boxes is not None and len(boxes) > 0:
        ...
        for i in range(len(confidences)):
            ...
            # angle ist jetzt der Rotationswinkel in Grad (-90 bis 0)
            # print(f"Rotationswinkel: {angle}°")
...
try:
    ...
    while True:
        ...
        # Calculate and display pose whenever shapes are detected
        """"if detected_shapes:
            pose = pose_processing()
            print(f"Calculated pose: {pose}")
        """
